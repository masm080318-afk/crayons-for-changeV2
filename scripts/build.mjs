...
function metrics({ title = true, link = false } = {}) {
  return `<section class="impact-band" id="numbers" aria-label="C4C impact"><div class="wrap">${title ? `<div class="section-heading"><h2>Our impact so far</h2>${link ? textLink('See Our Impact', '/impact/') : ''}</div>` : ''}<dl class="metrics"><div><dt>School supplies distributed</dt><dd>${impact.suppliesDistributed.toLocaleString('en-US')}</dd></div><div><dt>Students supported</dt><dd>${impact.studentsApproximate ? '<span class="approximate">Approximately</span>' : ''}${impact.studentsSupported.toLocaleString('en-US')}</dd></div></dl>${!title && link ? textLink('See Our Impact', '/impact/') : ''}</div></section>`;
}
